...
describe('版本号一致性（每轮随最新基线刷新）', () => {
  it('version.ts 与 package.json 同步为 V1.0-8 / 1.0.0-8', () => {
    expect(APP_VERSION).toBe('V1.0-8')
    const pkg = JSON.parse(src('package.json')) as { version: string }
    expect(pkg.version).toBe('1.0.0-8')
  })

  it('三份文档都记录了当前版本 V1.0-8', () => {
    for (const f of ['PRD.md', 'MENU_SPEC.md', 'DEV_PLAN_V2.md']) {
      expect(src(f), `${f} 未同步版本号`).toContain('V1.0-8')
    }
  })
})
