...
describe('第十七轮：版本号一致性', () => {
  it('version.ts 与 package.json 同步为 V1.0-7 / 1.0.0-7', () => {
    expect(APP_VERSION).toBe('V1.0-7')
    const pkg = JSON.parse(src('package.json')) as { version: string }
    expect(pkg.version).toBe('1.0.0-7')
  })

  it('三份文档都记录了本轮版本', () => {
    for (const f of ['PRD.md', 'MENU_SPEC.md', 'DEV_PLAN_V2.md']) {
      expect(src(f), `${f} 未同步版本号`).toContain('V1.0-7')
    }
  })
})
