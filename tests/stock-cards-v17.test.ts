...
describe('版本号一致性（每轮随最新基线刷新）', () => {
   ...
   *     V2.1-1 → 2.1.1 ／ V2.1-1.1 → 2.1.2 ／ V2.1-1.2 → 2.1.3 ／ V2.1-1.3 → 2.1.4 ／ V2.1-1.4 → 2.1.5 ／ V2.1-1.5 → 2.1.6 ／ V2.1-2 → 2.2.0 ／ V2.1-2.1 → 2.2.1 ／ V2.1-2.2 → 2.2.2 …
   *   它同时是 Service Worker 缓存名的来源，**每次发版都必须变**，否则老用户拿不到新版。
   */
  it('version.ts 与 package.json 同步为 V2.1-2.2 / 2.2.2', () => {
    expect(APP_VERSION).toBe('V2.1-2.2')
    const pkg = JSON.parse(src('package.json')) as { version: string }
    expect(pkg.version).toBe('2.2.2')
  })

  it('三份文档都记录了当前版本 V2.1-2.2', () => {
    for (const f of ['PRD.md', 'MENU_SPEC.md', 'DEV_PLAN_V2.md']) {
      expect(src(f), `${f} 未同步版本号`).toContain('V2.1-2.2')
    }
  })
})
